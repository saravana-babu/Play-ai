...
export default function Connect4() {
  ...
  const handleWin = async (result: Player | 'Draw') => {
    ...
    if (result === 'Yellow' && apiKeys[selectedLlm]) {
    ...
    if (user && isMounted.current) {
      try {
        await fetchApi('/history', {
          ...
          body: JSON.stringify({
            game_id: 'connect4',
            winner: result === 'Red' ? 'user' : result === 'Yellow' ? 'ai' : 'draw',
            funny_task: task,
  ...
  const handleColumnClick = (col: number) => {
    if (winner || !isUserNext || isAiThinking) return;
  ...
  const makeAiMove = async () => {
    ...
    try {
      ...
      if (availableCols.length > 0) {
        ...
        const newBoard = dropPiece(randomCol, 'Yellow', board);
        if (newBoard) {
          setBoard(newBoard);
          const result = checkWinner(newBoard);
        ...
        <div className="flex gap-2 mb-2">
          {Array(COLS).fill(null).map((_, colIndex) => (
            <button
              ...
              disabled={!!winner || !isUserNext || isAiThinking || !!board[0][colIndex]}
                ...
                <AnimatePresence>
                  {cell && (
                    <motion.div
                      ...
                      className={`absolute inset-0 rounded-full shadow-inner shadow-black/50 ${
                        cell === 'Red' ? 'bg-rose-500' : 'bg-amber-400'
                      }`}
                    />
                  )}
                </AnimatePresence>
        ...
        {winner && (
          ...
          >
            <div className={`inline-flex items-center gap-2 px-4 py-2 rounded-full font-bold text-lg ${
              winner === 'Red' ? 'bg-rose-500/20 text-rose-400' :
              winner === 'Yellow' ? 'bg-amber-500/20 text-amber-400' :
              'bg-slate-700 text-slate-300'
            }`}>
              {winner === 'Red' ? <><Trophy className="w-5 h-5" /> You Win!</> :
               winner === 'Yellow' ? <><Frown className="w-5 h-5" /> AI Wins!</> :
            ...
            <div className="flex gap-4 justify-center mt-4">
              ...
              <ShareButton 
                gameTitle="Connect 4" 
                winner={winner === 'Red' ? 'user' : winner === 'Yellow' ? 'ai' : 'draw'} 
                funnyTask={funnyTask} 
              />
            </div>
          </motion.div>
