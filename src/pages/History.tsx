              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
                <div className="flex items-center gap-3">
                  <div className={`p-2 rounded-xl ${
                    result.winner === 'user' ? 'bg-emerald-500/20 text-emerald-400' :
                    result.winner === 'ai' ? 'bg-rose-500/20 text-rose-400' :
                    'bg-slate-700 text-slate-400'
                  }`}>
                    {result.winner === 'user' ? <Trophy className="w-5 h-5" /> :
                     result.winner === 'ai' ? <Frown className="w-5 h-5" /> :
                ...
                <div className={`px-3 py-1 rounded-full text-sm font-semibold uppercase tracking-wider ${
                  result.winner === 'user' ? 'bg-emerald-500/10 text-emerald-400' :
                  result.winner === 'ai' ? 'bg-rose-500/10 text-rose-400' :
